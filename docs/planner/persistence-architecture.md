...
**Immutability & privacy:** original `task_events` rows are **never content-edited and never deleted** — no UPDATE/DELETE policy, `revoke` mutation, and a reject-mutation trigger rejecting every content-altering UPDATE and every DELETE for every role. The **sole** permitted mutation is PostgreSQL's referential cleanup — the FK-driven `actor_user_id` transition from a non-null user id to `NULL` on profile deletion, allowed only when every other event field is unchanged. This is referential cleanup, not an editable event mutation; the `actor_display` snapshot and all other content stay immutable. Payloads carry only sanitized domain facts (ids, before/after, reasons) — **never secrets or provider bodies**. Privacy is handled by a **redaction overlay, never by editing payloads** (§17). Retained long-term; partitioning deferred.
...
**As shipped (0046 — final).** Approach A was implemented as two functions, **`SECURITY INVOKER` with `set search_path = public`** and `EXECUTE` granted **only to `service_role`** (revoked from `public`/`anon`/`authenticated`) — *not* SECURITY DEFINER. `SECURITY INVOKER` is the correct fit because the sole caller, `service_role`, already bypasses RLS and holds exactly the required table grants (and holds only `INSERT, SELECT` on the append-only event tables, so the op itself cannot rewrite history); SECURITY DEFINER would have needlessly widened privilege. The verified actor still travels as an explicit envelope parameter. The two functions: **`apply_task_command(jsonb)`** (the atomic command-apply op — version/event-identity owned internally, structural command/event contract, single transaction) and **`apply_event_redaction(jsonb)`** (the append-only redaction-overlay writer; emits no task-event). Privileged erasure/restore remain deferred to a later migration.
