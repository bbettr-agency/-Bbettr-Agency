...
**Companion documents.** The [Planner Execution Model](./execution-model.md) defines **product behaviour** (how the Planner behaves for the user). This document defines **domain behaviour and invariants** (how the Tasks engine works with every page deleted). Future persistence, services, APIs, automations, and UI must conform to **both**.
