...
export async function getInvoiceRequests() {
  ...
  const { data } = await supabase
    ...
    .select(
      "id, amount, billing_type, status, created_at, deal_id, rep_id, quickbooks_invoice_number, quickbooks_invoice_id, quickbooks_customer_id, quickbooks_realm_id, quickbooks_email_status, quickbooks_emailed_at, quickbooks_last_attempt_at, invoiced_at, error, deals(business_name, contact_name, email, package, client_location)"
    )
    .order("created_at", { ascending: false });
  return data ?? [];
}
